...
| --- | --- | --- | --- | --- | --- | --- | --- |
| お問い合わせ受付（ユーザー向け） | WF01 | フォーム表示・初期入力 | 提供済みフォームUIの描画。バックエンドは `/` ルートで `contact.index` を返し、CSRF・meta情報が正しく挿入されていることを保証する。 | URL: GET `/`; 認証: 不要。 | 画面表示が成功し、後続APIへのアクセスが許可される。 | `ContactController@index` の実装維持とルーティング。フロントから呼ばれる `/api/categories` `/api/tags` が正常稼働していることが前提。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF02 | 入力内容確認 | 確認画面切替は提供済みJSが担うため、バックエンドはバリデーションルールを `StoreContactRequest` と共有し、APIのレスポンス形式を維持する。 | 同上。 | 確認画面でカテゴリ・タグ名称がAPIレスポンス通りに表示される。 | ルール変更時は `resources/js/validation-messages.js` と整合を取る。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF03 | お問い合わせ送信 | 提供済みUIが `POST /api/contacts` を呼び出すので、APIがJSONを受け取り `contacts` / `contact_tag` へ永続化しThanksへ遷移させる。 | URL: POST `/api/contacts`; 認証: 不要; `StoreContactRequest` に従う。 | 201 Createdを返し、送信元でThanksへのリダイレクト処理が可能。 | `ContactController@store`（API）のロジック維持、`Contact` モデル/`tags()` リレーション定義。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF04 | 完了画面表示 | `/thanks` ビューを返すのみ。UIは提供済み。 | URL: GET `/thanks`; 認証: 不要。 | 完了メッセージが表示されHOMEリンクで `/` 戻れる。 | `ContactController@thanks` のルーティング維持。 | 基本 |
| 問い合わせ運用（管理者向け） | AF01 | 問い合わせ検索・一覧 | 管理画面UIが `GET /api/contacts` に条件を渡すので、APIがバリデーションとフィルタリングを実施し `ContactResource` で返却する。 | 画面URL: GET `/admin` （auth必須）・AJAX: GET `/api/contacts`（現行仕様では認証無し）。検索キーワード、性別（0=全件/1-3）、カテゴリID、日付をクエリで受け取り、1ページ7件でページングする。 | JSON結果をUIが描画し、`meta` にページネーション情報（現在ページ、links 等）を含める。 | `IndexContactRequest` で入力チェック、`ContactController@index`（API）で `paginate(7)` を利用し `ContactResource` を返す。 | 基本 |
| 問い合わせ運用（管理者向け） | AF02 | 詳細参照＆削除 | UIモーダルが `GET /api/contacts/{id}` と `DELETE /api/contacts/{id}` を利用する。APIはカテゴリ・タグをロードし、削除要求で204を返す。 | 詳細取得は公開API（認証無し）を利用、削除は同じく公開APIから呼ばれる想定。 | 詳細描画用のJSON（カテゴリ・タグ付き）と削除成功レスポンス。 | API show/destroy実装 (`app/Http/Controllers/Api/ContactController.php:59,64`) を維持し、削除時はcascadeで関連タグを削除する。 | 基本 |
| 問い合わせ運用（管理者向け） | AF03 | CSVエクスポート | 管理画面のエクスポートボタンが `/contacts/export` を開くため、同じ条件で絞り込んだCSVを返す。 | URL: GET `/contacts/export` + query（`auth` 必須）。検索条件はAF01と同じ。 | Excel互換のCSV（BOM付）がダウンロードされる。 | `ExportContactRequest` / `ContactController@export` が検索条件とCSV整形を担う。 | 応用 |
| 問い合わせ運用（管理者向け） | AF04 | タグマスタ管理 | UIのCRUD操作が `/api/tags` 系エンドポイントを呼び出す。APIは作成/更新/削除を行い、エラーは422 JSONで返す。 | name必須/<=50/ユニーク（`StoreTagRequest` / `UpdateTagRequest`）。 | タグ一覧が最新状態になる。 | `TagController`（API）とバリデーションルール維持。 | 応用 |
| 管理ユーザー認証・アカウント管理 | AU01 | 管理ユーザー登録 | Fortify登録画面（提供済み）から `CreateNewUser` が呼ばれ、ユーザーを作成。 | URL: GET/POST `/register`; 入力ルールは `CreateNewUser` 参照。 | 成功時 `/admin` に遷移。 | Fortify設定とシーディング。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU02 | ログイン/ログアウトと保護 | Fortifyログインフォームを使用し、authミドルウェアで `/admin` や `/contacts/export` を保護。API（`/api/*`）は現行仕様では公開。 | URL: GET/POST `/login`, POST `/logout`; RateLimit: 5/分。 | 認証OKで `/admin` 表示、logoutでセッション破棄。 | Fortify設定＋ `routes/web.php` の auth グループ維持。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU03 | パスワード再設定 | Fortifyのリセットメールと `ResetUserPassword` でパスワード更新。 | 入力：メール/トークン/新パスワード。 | `password_reset_tokens` に記録、更新成功。 | Fortify features とトークンMigration。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU04 | プロフィール／パスワード更新 | Fortifyのプロファイル機能。`UpdateUserProfileInformation` / `UpdateUserPassword` を通じて `users` を更新。 | 認証済ユーザー／現行PW必須。 | name/email/password が更新。 | Fortifyアクション維持。 | 応用 |
| 管理ユーザー認証・アカウント管理 | AU05 | 二要素認証 | Fortify Two-Factor機能を使用。提供済みUIで設定し、`users` の2FAカラムを使用。 | TOTPコード等。 | 有効化時にtwo_factor情報を保持し、ログイン時にチャレンジ。 | Fortify設定・マイグレーション。 | 応用 |
