...
## 機能要件
| EPIC名 | 機能ID | 機能名 | 概要（ビジネスロジック） | システムロジック（バックエンド要件） | 入力条件/制約 | 出力条件/期待結果 | 実装要件 | 区分 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| お問い合わせ受付（ユーザー向け） | WF01 | フォーム表示・初期入力 | ユーザーが `/` で問い合わせフォーム全項目を閲覧し、入力を開始できる。 | `/` ルートで `contact.index` を返し、CSRF/metaタグを埋め込む。カテゴリ／タグ候補を取得する公開APIが稼働していること。 | URL: GET `/`; 認証: 不要。 | 画面がエラー無く表示される。 | `ContactController@index` とルーティング。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF02 | 入力内容確認 | 入力値を画面内で確認し、カテゴリ・タグ名称を含めて表示する。 | バックエンドは `StoreContactRequest` と同一ルールを提供し、APIレスポンス形式（カテゴリID/タグIDに紐づく名称）が変わらないよう維持する。 | WF01と同じ。 | 確認画面で名称が表示され、修正へ戻れる。 | ルール変更時はJSと同期。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF03 | お問い合わせ送信 | ユーザーが入力内容を送信し、問い合わせと任意タグを登録できる。 | `POST /api/contacts` がJSONを受け取り、`contacts`・`contact_tag` テーブルへ保存し201を返す。 | URL: POST `/api/contacts`; 認証: 不要; `StoreContactRequest` に従う。 | 送信成功でThanks遷移、エラー時に422 JSON。 | APIコントローラ/モデル/バリデーション。 | 基本 |
| お問い合わせ受付（ユーザー向け） | WF04 | 完了画面表示 | 送信後に完了メッセージを提示し、HOMEへ戻す導線を提供する。 | `/thanks` ルートで提供済みビューを返す。 | URL: GET `/thanks`; 認証: 不要。 | メッセージ表示とHOMEリンク。 | `ContactController@thanks`。 | 基本 |
| 問い合わせ運用（管理者向け） | AF01 | 問い合わせ検索・一覧 | 管理者が認証後の画面で条件検索し、7件ごとの一覧とページ切り替えができる。 | `/admin` は `auth` で保護。AJAXで `GET /api/contacts` を呼び、`IndexContactRequest` で入力を検証し `paginate(7)`＋`ContactResource`（`meta` 付き）を返す。 | 画面URL: GET `/admin`（要ログイン）、API: GET `/api/contacts`（公開）。キーワード/性別/カテゴリ/日付/ページ。 | JSON結果に `data` と `meta.links` が含まれ、UIが描画。 | APIコントローラ/リソース/バリデーション。 | 基本 |
| 問い合わせ運用（管理者向け） | AF02 | 詳細参照＆削除 | 一覧から詳細を閲覧し、不要な問い合わせを削除できる。 | `GET /api/contacts/{id}` がカテゴリ・タグ情報入りで返し、`DELETE /api/contacts/{id}` が204で削除。Cascadeで `contact_tag` も削除。 | APIは公開。 | 詳細表示用JSONと削除成功レスポンス。 | API show/destroy実装。 | 基本 |
| 問い合わせ運用（管理者向け） | AF03 | CSVエクスポート | 検索条件に一致する問い合わせ一覧をCSVとしてダウンロードできる。 | 認証済み管理者が `/contacts/export` を実行。`ExportContactRequest` で条件を受け、`Contact` を絞り込みBOM付きCSVで出力。 | URL: GET `/contacts/export` + query（auth必須）。 | CSVファイルがダウンロード。 | `ContactController@export`。 | 応用 |
| 問い合わせ運用（管理者向け） | AF04 | タグマスタ管理 | 管理者がタグを追加・更新・削除し、問い合わせフォームで利用する候補をメンテナンスできる。 | `/api/tags` 系の公開APIが `Store/UpdateTagRequest` に従いCRUDを実行。 | name必須/<=50/ユニーク。 | APIレスポンスを受けUIが反映。 | `TagController`（API）。 | 応用 |
| 管理ユーザー認証・アカウント管理 | AU01 | 管理ユーザー登録 | 管理者がFortifyの登録画面から新しい管理ユーザーを作成できる。 | `CreateNewUser` が入力を検証し `users` に保存。 | URL: GET/POST `/register`; ルールは `CreateNewUser`。 | 作成後 `/admin` へ。 | Fortify設定。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU02 | ログイン/ログアウトと保護 | 管理者がログインフォームから認証し、ログアウトも可能。 | Fortifyログインを利用し、`auth` ミドルウェアで `/admin` `/contacts/export` を保護。APIは公開。 | URL: GET/POST `/login`, POST `/logout`; RateLimit:5/分。 | ログイン成功→`/admin`、ログアウト→セッション破棄。 | Fortify設定・ルーティング。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU03 | パスワード再設定 | 管理者がメール経由でパスワードをリセットできる。 | Fortifyのリセットフローと `ResetUserPassword` を利用。 | メール/トークン/新PW。 | パスワード更新。 | Fortify設定。 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU04 | プロフィール／パスワード更新 | 管理者が自身の名前・メール・パスワードを更新できる。 | Fortifyの `UpdateUserProfileInformation` / `UpdateUserPassword` を利用。 | 認証済ユーザー。 | `users` 更新。 | Fortify設定。 | 応用 |
| 管理ユーザー認証・アカウント管理 | AU05 | 二要素認証 | 管理者が二要素認証を有効化・利用できる。 | FortifyのTwo-Factor機能と `users` テーブルの2FAカラムを使用。 | TOTPコード等。 | 2FA有効化/認証。 | Fortify設定。 | 応用 |
