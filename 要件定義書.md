...
| --- | --- | --- | --- | --- |
| first_name / last_name | string | 必須 | string, max:255 | 姓・名 |
| gender | integer | 必須 | in:1,2,3 | 性別 |
| email | string | 必須 | string, email, max:255 | メールアドレス |
| tel | string | 必須 | regex:/^[0-9]{10,11}$/ | 電話番号（ハイフン無し） |
| address | string | 必須 | string, max:255 | 住所 |
| building | string | 任意 | string, max:255 | 建物名 |
| category_id | integer | 必須 | exists:categories,id | 問い合わせ分類 |
| detail | string | 必須 | string, max:120 | 問い合わせ内容 |
| tag_ids[] | array<int> | 任意 | exists:tags,id | 任意タグID |
...
**リクエストパラメータ**
- `contact`（path param, integer, 必須）

**レスポンス**
| ステータス | 説明 |
| --- | --- |
| 200 OK | `ContactResource` を返却 |
...
| パラメータ名 | 型 | 必須 | バリデーション | 説明 |
| --- | --- | --- | --- | --- |
| name | string | 必須 | required, string, max:50, unique:tags,name | タグ名 |

**レスポンス**
| ステータス | 説明 |
| --- | --- |
| 201 Created | 登録成功（空レスポンス） |
| 422 Unprocessable Entity | バリデーションエラー |
...
| パラメータ名 | 型 | 必須 | バリデーション | 説明 |
| --- | --- | --- | --- | --- |
| name | string | 必須 | required, string, max:50, unique:tags,name (対象ID除外) | タグ名 |
