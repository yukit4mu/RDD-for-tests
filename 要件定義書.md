...
| SCR04 | 管理画面 | GET `/admin` | 認証済みユーザー向けの提供済みUI。検索やモーダルのイベントはJSが担うため、バックエンドは `/admin` ビュー提供とAPI/CSVエンドポイントの整合性を確保する。`routes/web.php:25` |
...
| 問い合わせ運用（管理者向け） | AF01 | 問い合わせ検索・一覧 | 管理画面UIが `GET /api/contacts` に条件を渡すので、APIがバリデーションとフィルタリングを実施し `ContactResource` で返却する。 | URL: GET `/admin` (auth) + AJAXで `/api/contacts`; 認証: `/admin`は必須、APIは現行仕様通り公開。 | JSON結果をUIが描画し、ページネーション情報も含まれる。 | `IndexContactRequest` / `ContactController@index`（API）/ `ContactResource` の整合性を保つ。 | 基本 |
| 問い合わせ運用（管理者向け） | AF02 | 詳細参照＆削除 | UIモーダルが `GET /api/contacts/{id}` と `DELETE /api/contacts/{id}` を利用する。APIはカテゴリ・タグをロードし、削除要求で204を返す。 | contact_id path param。削除時はUIで確認済み。 | 詳細描画用のJSONと削除成功レスポンス。 | API show/destroy実装 (`app/Http/Controllers/Api/ContactController.php:59,64`) を維持。 | 基本 |
| 問い合わせ運用（管理者向け） | AF03 | CSVエクスポート | 管理画面のエクスポートボタンが `/contacts/export` を開くため、同じ条件で絞り込んだCSVを返す。 | URL: GET `/contacts/export` + query; 認証: 必須。 | Excel互換のCSVがダウンロードされる。 | `ExportContactRequest` / `ContactController@export` が検索条件とCSV整形を担う。 | 応用 |
...
| 管理ユーザー認証・アカウント管理 | AU02 | ログイン/ログアウトと保護 | Fortifyログインフォームを使用し、authミドルウェアで `/admin` や `/contacts/export` を保護。 | URL: GET/POST `/login`, POST `/logout`; RateLimit: 5/分。 | 認証OKで `/admin` 表示、logoutでセッション破棄。 | Fortify設定＋ `routes/web.php` の auth グループ維持。 | 基本 |
