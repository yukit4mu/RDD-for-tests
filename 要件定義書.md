...
| 管理ユーザー認証・アカウント管理 | AU03 | パスワード再設定 | 管理者がメール経由でパスワードをリセットできる。 | - Fortifyリセットフォーム | パスワード更新。 | - Fortifyのリセットフローを有効化。<br>- `ResetUserPassword` が新PWを保存。 | - `app/Actions/Fortify/ResetUserPassword`<br>- Fortify設定 | 基本 |
| 管理ユーザー認証・アカウント管理 | AU04 | プロフィール／パスワード更新 | 管理者が自身の名前・メール・パスワードを更新できる。 | - 認証済ユーザー | `users` テーブルが更新。 | - `UpdateUserProfileInformation` / `UpdateUserPassword` を利用。<br>- バリデーションエラーはFortifyのフォームバッグへ返却。 | - `app/Actions/Fortify/UpdateUserProfileInformation`<br>- `app/Actions/Fortify/UpdateUserPassword` | 応用 |
| 管理ユーザー認証・アカウント管理 | AU05 | 二要素認証 | 管理者が二要素認証を有効化・利用できる。 | - TOTPコード等 | 2FA有効化・認証フローが利用可能。 | - Fortify Two-Factor 機能を有効化。<br>- `users` テーブルの2FAカラムを使用。 | - `config/fortify.php`<br>- `database/migrations/2014_10_12_200000_add_two_factor_columns_to_users_table.php` | 応用 |
